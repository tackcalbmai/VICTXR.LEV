...
async function assertEditorialIntro(name, viewport) {
  ...
  await page.waitForFunction(() => document.querySelector('[data-editorial-intro]')?.getAttribute('data-editorial-phase') === 'wrong', undefined, { timeout: 2200 });
  ...
  await page.waitForFunction(() => document.querySelector('[data-editorial-intro]')?.getAttribute('data-editorial-phase') === 'good', undefined, { timeout: 2200 });
  ...
  await page.waitForFunction(() => document.querySelector('[data-editorial-intro]')?.getAttribute('data-editorial-phase') === 'break', undefined, { timeout: 2200 });
  ...
  await page.waitForFunction(() => document.querySelector('[data-editorial-intro]')?.getAttribute('data-editorial-phase') === 'different', undefined, { timeout: 2200 });
  ...
  await page.waitForFunction(() => document.querySelector('[data-editorial-intro]')?.getAttribute('data-editorial-phase') === 'reveal', undefined, { timeout: 2200 });
  await page.waitForTimeout(240);
  await page.screenshot({ path: `${outDir}/${name}-editorial-reveal.png`, fullPage: false });

  await page.waitForSelector('[data-editorial-intro]', { state: 'detached', timeout: 2200 });
  await page.waitForFunction(() => !document.documentElement.classList.contains('is-editorial-intro'), undefined, { timeout: 2200 });

  const elapsed = Date.now() - started;
  if (elapsed < 3450 || elapsed > 4550) throw new Error(`${name} intro duration ${elapsed}ms is outside the intended ~4s window`);
...
const desktopDuration = await assertEditorialIntro('desktop-1366', { width: 1366, height: 768 });
const mobileDuration = await assertEditorialIntro('mobile-393', { width: 393, height: 852 });
if (Math.abs(desktopDuration - mobileDuration) > 250) {
  throw new Error(`Desktop/mobile editorial intro timing diverged (${desktopDuration}ms vs ${mobileDuration}ms)`);
}
...
await browser.close();
console.log(`Editorial NORMAL → WRONG → VICTXR intro passes with matched timing (${desktopDuration}ms / ${mobileDuration}ms).`);
