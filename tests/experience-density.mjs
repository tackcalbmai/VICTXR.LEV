  ...
  for (const path of routes) {
    ...
    if (path === '/') {
      ...
      const work = await page.evaluate(() => {
        ...
        return title && catrin && anelika ? { title: rect(title), catrin: rect(catrin), anelika: rect(anelika) } : null;
      }, rect);
      ...
      const perspective = await page.evaluate(() => {
        const spans = [...document.querySelectorAll('.home-v2-perspective h2 span')].map((el) => rect(el.getBoundingClientRect()));
        const bottom = document.querySelector('.home-v2-perspective__bottom')?.getBoundingClientRect();
        return bottom && spans.length ? { spans, bottom: rect(bottom) } : null;
      }, rect);
    ...
    if (path === '/contact/') {
      ...
      const intents = await page.locator('.contact-intent').evaluateAll((nodes) => nodes.map((node) => rect(node.getBoundingClientRect())), rect);
      ...
      const brief = await page.evaluate(() => {
        const rows = [...document.querySelectorAll('.contact-brief__row')].map((el) => rect(el.getBoundingClientRect()));
        const controls = [...document.querySelectorAll('.contact-brief__actions > a, .contact-brief__actions > button')].map((el) => rect(el.getBoundingClientRect()));
