...
for (const current of cases) {
  ...
  const homeNav = await page.locator('.site-nav--desktop a').evaluateAll((links) => links.slice(0, 3).map((link) => link.getAttribute('href')));
  ...
  const heroActions = await page.locator('.hero__actions a').evaluateAll((links) => links.map((link) => link.getAttribute('href')));
  ...
  const caseNav = await page.locator('.site-nav--desktop a').evaluateAll((links) => links.slice(0, 3).map((link) => link.getAttribute('href')));
